...
date: 2024-11-30
description: Vintage is a hard difficulty Windows machine.
published: true
...
![Initial Nmap Scan](/assets/htb/1.png)
...
![Initial Nmap Scan](/assets/htb/2.png)
...
```
![Initial Nmap Scan](/assets/htb/3.png)
...
As we can see here, no **DONT_REQ_PREAUTH** flag there, which means Pre Authentication is enabled.
...
Knowing that we have **GenericAll** on these 3 accounts, we need to modify these accounts and set **DONT_REQ_PREAUTH** as preparation for **ASREPRoasting**.
...
After a lottttt of enumerations, I searched through **DPAPI (Data Protection API)**.
...
There is an AV on the machine, so we couldn’t run **SharpDPAPI.exe** for example.  
So I did it manually by following [this article](https://www.synacktiv.com/en/publications/windows-secrets-extraction-a-summary).
...
![Initial Nmap Scan](/assets/htb/5.png)
![Initial Nmap Scan](/assets/htb/4.png)
Looks like a free win here.

There are two possible ways to escalate privileges:

- **RBCD with a Computer Account**
- **Add an SPN to RBCD**
